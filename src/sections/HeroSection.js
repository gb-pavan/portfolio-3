...
const HeroSection = () => {
  return (
    <section className="w-full px-4 sm:px-8 md:px-16 lg:px-24 py-16">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-y-12 gap-x-12">
        {/* Left Section */}
        <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left ml-8">
          <span
            className="text-white text-xl font-semibold px-5 py-1"
            style={{
              background: 'linear-gradient(to right, #32F6F6, #A6A79F)',
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center gap-4">
            <button className="font-segoe font-bold text-lg px-6 py-2 bg-download-gradient rounded-custom-btn border border-white text-white">
              Download CV
            </button>

            <button className="font-segoe font-bold text-lg px-6 py-2 bg-contact-gradient rounded-custom-btn border border-white text-white">
              Get In Touch!
            </button>
          </div>
        </div>
